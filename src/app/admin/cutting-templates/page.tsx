...
import { GarmentType, CuttingTemplate, CuttingTemplateTask } from "@/types";
import { getAllCuttingTemplates, saveCuttingTemplate } from "@/lib/cuttingTemplates";
...
export default function AdminCuttingTemplatesPage() {
    const [templates, setTemplates] = useState<CuttingTemplate[]>([]);
    const [loading, setLoading] = useState(true);
    const [saving, setSaving] = useState<string | null>(null);
    const [selectedGarment, setSelectedGarment] = useState<GarmentType>("blouse");
    const [editedTasks, setEditedTasks] = useState<CuttingTemplateTask[]>([]);
    ...
    const loadTemplates = async () => {
        try {
            const data = await getAllCuttingTemplates();
            setTemplates(data);
        } catch (error) {
    ...
    const handleAddTask = () => {
        const newTask: CuttingTemplateTask = {
            taskName: "",
    ...
    const handleTaskChange = (index: number, field: keyof CuttingTemplateTask, value: string | number | boolean) => {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    <div className="mb-6">
                        <h1 className="text-2xl sm:text-3xl font-bold text-gray-200 mb-2 flex items-center gap-2">
                            ...
                            <span>Cutting Templates</span>
                        </h1>
                        <p className="text-slate-400">
                            Define cutting tasks for each garment type
                        </p>
                    </div>
                    ...
                    <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-semibold text-gray-200">
                                {GARMENT_LABELS[selectedGarment]} Cutting Tasks
                            </h2>
                    ...
                    <div className="mt-6 bg-blue-900/20 border border-blue-800 rounded-xl p-4">
                        <h3 className="font-medium text-blue-300 mb-2">About Cutting Templates</h3>
                        <ul className="text-sm text-blue-400 space-y-1">
                            <li>• Templates define which cutting tasks are created for each garment type</li>
                            ...
                            <li>• &quot;Required&quot; tasks must be completed for the cutting stage to finish</li>
                        </ul>
                    </div>
                </div>
