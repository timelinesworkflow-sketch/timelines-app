...
import { Order, CuttingTask, User, getGarmentDisplayName, OrderItem } from "@/types";
...
export default function CuttingPage() {
    ...
    const handleCompleteTask = async (taskId: string) => {
        setActionLoading(taskId);
        try {
            await completeCuttingTask(taskId);
    ...
    const handleCompleteCutting = async (orderId: string, activeStages: string[]) => {
        setActionLoading(orderId);
        try {
            const nextStage = getNextStage("cutting", activeStages);
            ...
            const updateData: Partial<Order> = {
                currentStage: nextStage || "stitching"
            };
            ...
            setToast({ message: `Cutting complete! Order moved to ${nextStage || "stitching"}`, type: "success" });
            loadData();
        } catch (error) {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div>
                                ...
                                <p className="text-sm text-slate-400 mt-1">
                                    {isCuttingStaff
                                        ? "Manage your assigned cutting tasks"
                                        : "Monitor all active cutting tasks"}
                                </p>
                            </div>
                            <button
                                onClick={loadData}
                    ...
                    {loading ? (
                        ...
                        <div className="bg-slate-800 border border-slate-700 rounded-xl p-12 text-center">
                            ...
                            <h3 className="text-lg font-medium text-gray-200 mb-2">No active cutting orders</h3>
                            <p className="text-slate-400">Orders will appear here once they complete the marking stage.</p>
                        </div>
                    ) : (
                                    ...
                                    <div key={order.orderId} className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
                                        ...
                                        {canAssign && allTasksApproved && (
                                            <div className="px-4 py-3 bg-emerald-900/20 border-t border-emerald-900/30 flex justify-end">
                                                <button
                                                    onClick={() => handleCompleteCutting(order.orderId, order.activeStages)}
